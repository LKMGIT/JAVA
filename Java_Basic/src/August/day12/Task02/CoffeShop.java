...
public class CoffeShop {
	...
	private static void getSalesInfo(Beverage[] beverage) {
		for(int i = 0; i< beverage.length; i ++){
			if(beverage[i] instanceof Coffee coffee){
				System.out.println(i+1 +"번째 판매 음료는 "+ coffee.getName() + "이며, 가격은 :" + coffee.getPrice());
			} else if (beverage[i] instanceof  Tea tea) {
				System.out.println(tea.getAmount() +"번째 판매 음료는 "+ tea.getName() + "이며, 가격은 :" + tea.getPrice());
			}
		}
	}
